...
from everyclass.identity.config.default import Config as DefaultConfig
...
def get_config():
    ...
    if _config_inited:
        ...
        if mode == 'PRODUCTION':
            from everyclass.identity.config.production import ProductionConfig
            _override_config = ProductionConfig
            MixedConfig.CONFIG_NAME = 'production'
        elif mode == 'DEVELOPMENT':
            from everyclass.identity.config.development import DevelopmentConfig
            _override_config = DevelopmentConfig
            MixedConfig.CONFIG_NAME = 'development'
        elif mode == 'STAGING':
            from everyclass.identity.config.staging import StagingConfig
            _override_config = StagingConfig
            MixedConfig.CONFIG_NAME = 'staging'
        elif mode == 'TESTING':
            from everyclass.identity.config.testing import TestingConfig
        ...
        try:
            from everyclass.identity.config.default import LazyRefType
